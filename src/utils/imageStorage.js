...
const SECONDARY_QUALITY = 0.6; // If still too large, drop to 60%
...
export async function uploadImage(file) {
  ...
  try {
    ...
    console.log(
      `✓ Image compressed: ${Math.round(originalSize / 1024)}KB → ${Math.round(compressedSize / 1024)}KB (${savings}% saved)`,
    );
  } catch (compressErr) {
  ...
  if (!client) {
    const msg =
      "❌ CRITICAL: Supabase not configured. Images cannot upload. Check VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY environment variables.";
    console.error(msg);
    throw new Error(msg);
  }
  ...
  try {
    ...
    console.log(
      `⬆ Uploading to Storage: ${filename} (${Math.round(compressedSize / 1024)}KB)...`,
    );
